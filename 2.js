let data = `13-16 k: kkkkkgmkbvkkrskhd
    5-6 p: qpppvzp
    3-4 p: psppxhlfpvkh
...
let tempObject = { min: 0, max: 0, char: null, string: null };
let returnArray = [];
arrays.forEach((el) => {
  let parts = el.split(": ");
  tempObject.string = parts[1];
  let subparts = parts[0].split("-");
  tempObject.min = parseInt(subparts[0].replace(/\s/g, ""));
  let maxparts = subparts[1].split(" ");
  tempObject.max = parseInt(maxparts[0]);
  tempObject.char = maxparts[1];
  returnArray.push(tempObject);
  tempObject = {};
});
function first() {
  console.log(
    returnArray.filter((el) => {
      let length = (el.string.match(new RegExp(el.char, "g")) || []).length;
      if (length >= el.min && length <= el.max) {
        return true;
      } else {
        return false;
      }
...
function second() {
  console.log(
    returnArray.filter((el) => {
      let flag1 = el.string[el.min - 1] == el.char;
      let flag2 = el.string[el.max - 1] == el.char;
      if (flag1 && flag2) {
        return false;
      } else if (flag1 || flag2) {
        return true;
