let input = `muted coral bags contain 1 bright magenta bag, 1 dim aqua bag.
...
let sampleInput = `light red bags contain 1 bright white bag, 2 muted yellow bags.
...
function one() {
  ...
  let object = { container: null, bags: [] };
  let arrayOfObjects = [];
  lines.forEach((line) => {
    if (line.indexOf("no other") === -1) {
      let parts = line.split(/ bags contain /gm);
      object.bags = parts[1].split(/, /);
      object.container = parts[0];
      arrayOfObjects.push(object);
      object = { container: null, bags: [] };
    }
  });
  let map = new Map();
  //arrayOfObjects.find(el=>el.)
  arrayOfObjects.forEach((el) => {
    let tempArray = [];
    el.bags.forEach((bag) => {
      bag = bag.substring(2);
      bag = bag.replace(/\./g, "");
      bag = bag.replace(/ bags/g, "");
      bag = bag.replace(/ bag/g, "");
      tempArray.push(bag);
    });
    map.set(el.container, tempArray);
    //console.log(map);
    //console.log(el.bags);
  });
  let value = recursive(map, "shiny gold", []);
  //console.log(value.sort());
  console.log(value.length);
  //console.log(arrayOfObjects);
  function recursive(map, searchedValue, returnValue) {
    let findFlag = false;
    map.forEach((value, key) => {
      if (value.includes(searchedValue)) {
        findFlag = true;
        if (!returnValue.includes(key)) {
          returnValue.push(key);
        }
        recursive(map, key, returnValue);
      }
    });
    if (!findFlag) {
    }
...
function two() {
  ...
  arrayOfObjects.forEach((el) => {
    let tempArray = [];
    el.bags.forEach((bag) => {
      let amount = parseInt(bag.split(/\s/gm)[0]);
      bag = bag.substring(2);

      bag = bag.replace(/\./g, "");
      bag = bag.replace(/ bags/g, "");
      bag = bag.replace(/ bag/g, "");
      tempArray.push({ amount: amount, bags: bag });
    });
    map.set(el.container, tempArray);
    //console.log(el.bags);
  });
  let array = [];
  let value = recursive(map, "shiny gold", 1, 1);
  console.log(array);
  console.log(array.reduce((a, b) => a + b));
  function recursive(map, searchedValue, count, levelMultiple, ) {
    let data = map.get(searchedValue);
    let tempCount=count
    if (data !== undefined) {
      data.forEach((el) => {
        tempCount = el.amount * levelMultiple;
        console.log(
          searchedValue +
            "-" +
            JSON.stringify(el) +
        ...
        recursive(map, el.bags, tempCount, tempCount );
      });
    } else {
      return tempCount;
    }
