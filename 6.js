...
al
daevb`;
function returnArrayOfObjects(input) {
  lines = input.split(/[\r\n]/gm);
  let blankFlag = false;
  let tempObject = { answers: [], people: "" };
  let arrayOfObjects = [];
  let answers = [];
  lines.forEach((line, index) => {
    if (line === "") {
      arrayOfObjects.push(tempObject);
      tempObject = { answers: [], people: "" };
      blankFlag = true;
    } else {
      tempObject.answers.push(line);
  ...
  arrayOfObjects.forEach((object) => {
    object.answers.forEach((answer) => {
      for (let i = 0; i < answer.length; i++) {
        ...
        if (object.people.indexOf(char) === -1) {
          object.people += char;
        }
      }
    });
    count += object.people.length;
  });
...
function one(input) {
  console.log(returnArrayOfObjects(input));
  console.log(returnCount(input));
}
function two(input) {
  let array = returnArrayOfObjects(input);
  array.forEach((object) => {
    object.countOfAllYes = 0;
    let fAnswer = object.answers[0];
    for (let i = 0; i < fAnswer.length; i++) {
      let char = fAnswer.charAt(i);
      let yesFlag = true;
      object.answers.forEach((a, index) => {
        if (index == 0) {
          return;
        }
        if (a.indexOf(char) == -1) {
      ...
      if (yesFlag) {
        object.countOfAllYes++;
      }
    }
  });
  let count = 0;
  array.forEach((el) => {
    count += el.countOfAllYes;
  });
  console.log(array);
  console.log(count);
}
