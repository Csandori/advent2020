...
function one(input) {
  ...
  let objectArray = [];
  inputProcessed.forEach((line, index) => {
    if (line === "") {
      // egy sor kimaradt
      newLineFlag = true;
    } else {
      ...
      newLineFlag = false;
      let splitData = line.split(/\s/g);
      splitData.forEach((value) => {
        let splitted = value.split(":");
        tempObject[splitted[0]] = splitted[1];
      });
    }
    if (index === inputProcessed.length - 1) {
      objectArray.push(tempObject);
    }
  });
...
function two() {
  ...
  function validate(object) {
    let byrValue = parseInt(object.byr);
    let iyrValue = parseInt(object.iyr);
    let eyrValue = parseInt(object.eyr);
    let hgtType = object.hgt.substring(object.hgt.length-2);
    let hgtValue = parseInt(object.hgt.substring(0,object.hgt.length-2));
    if (!(object.byr.length == 4 && byrValue >= 1920 && byrValue <= 2002)) {
      console.log("byr");
      console.log(object.byr);
      return false;
    }
    if (!(object.iyr.length == 4 && iyrValue >= 2010 && iyrValue <= 2020)) {
      console.log("iyr");
      console.log(object.iyr);
      return false;
    }
    if (!(object.eyr.length == 4 && eyrValue >= 2020 && eyrValue <= 2030)) {
      console.log("eyr");
      console.log(object.eyr);
      return false;
    }
    switch (hgtType) {
      case "cm":
        if (!(hgtValue >= 150 && hgtValue <= 193)) {
          console.log("hgtWrongCm");
          console.log(hgtValue);
          return false;
        }
        break;
      case "in":
        if (!(hgtValue >= 59 && hgtValue <= 76)) {
          console.log("hgtWrongIn");
          console.log(hgtValue);
          return false;
        }
        break;
    ...
    if (object.hcl.charAt(0) === "#") {
      let hclValue = object.hcl.substring(1);
      if (hclValue.length != 6) {
          console.log("hclValuelength");
          console.log(hclValue.length);
        return false;
      } else {
        for (let i = 0; i < 6; i++) {
          let char = hclValue.charAt(i);
          if (availableChars.indexOf(char) === -1) {
            console.log("hcl wrong char");
            console.log(object.hcl);
            return false;
          }
        }
      }
    } else {
      console.log("hclWrongStart");
      console.log(object.hcl);
      return false;
    }

    let availableEyeColors = ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"];
    if (availableEyeColors.indexOf(object.ecl) === -1) {
        console.log("Wrong eye color");
        console.log(object.ecl);
      return false;
    }
    if (!(object.pid.length === 9 && !isNaN(object.pid))) {
        console.log("object.pid wrogn")
